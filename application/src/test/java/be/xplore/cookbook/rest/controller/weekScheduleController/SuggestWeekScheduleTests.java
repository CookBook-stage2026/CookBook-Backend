...
class SuggestWeekScheduleTests extends BaseIntegrationTest {
    ...
    void suggestWeekSchedule_shouldReturnWeekScheduleWithSevenDays_whenValidRequest() throws Exception {
        ...
        performSuggest(WEEK_START_DATE)
    ...
    void suggestWeekSchedule_shouldReturnWeekSchedule_whenAdjacentWeekSchedulesExist() throws Exception {
        ...
        createWeekSchedule(user, Map.of(DayOfWeek.MONDAY, recipe), WEEK_START_DATE.minusWeeks(1));
        createWeekSchedule(user, Map.of(DayOfWeek.MONDAY, recipe), WEEK_START_DATE.plusWeeks(1));
    ...
    @Test
    void suggestWeekSchedule_shouldReturn400_whenWeekStartDateIsNotAMonday() throws Exception {
        createUser();

        performSuggest(WEEK_START_DATE.plusDays(1))
                .andExpect(status().isBadRequest());
    }

    @Test
    void suggestWeekSchedule_shouldReturn400_whenWeekStartDateIsMissing() throws Exception {
        createUser();

        getMockMvc().perform(get("/api/schedules/suggest/week")
                        .with(validJwt())
    ...
    void suggestWeekSchedule_shouldReturn401_whenNotAuthenticated() throws Exception {
        getMockMvc().perform(get("/api/schedules/suggest/week")
                        .param("weekStartDate", WEEK_START_DATE.toString())
                        .contentType(MediaType.APPLICATION_JSON))
    ...
    private ResultActions performSuggest(LocalDate weekStartDate) throws Exception {
        return getMockMvc().perform(
                get("/api/schedules/suggest/week")
